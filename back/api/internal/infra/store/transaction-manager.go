...
func (t *TransactionManager) StartTransaction(id, path string, pages int) *convert.Transaction {
	tran := convert.NewTransaction(id, path, pages)
...
func (t *TransactionManager) getTran(id string) (*convert.Transaction, error) {
	val, ok := t.transactions.Load(id)
	if !ok {
		return nil, fmt.Errorf("transaction doesn't exists")
	}

	tran, ok := val.(*convert.Transaction)
	if !ok {
